...
static const EnvWrite_t txOvsp[2] = {{0x74, 0x40}, {0x72, 0x01}}; // Page 1
...
static double CalcTemperature() {
	 uint32_t temp_adc;
 	 double var1, var2, temp_final,temp_comp;
 	 // Refer to datasheet 3.3.1 and Table 11
 	 // Combine 3 bytes into 20-bit ADC value to use in calculations
 	 temp_adc = tempData[0] << 12 | tempData[1] << 4 | tempData[2] >> 4 ;
 	 
	 //Calculates the temperature compensation formula from the datasheet.
	 var1 = (((double)temp_adc / 16384.0) - ((double)par_t[0] / 1024.0)) * (double)par_t[1];
 	 var2 = ((((double)temp_adc / 131072.0) - ((double)par_t[0] / 8192.0)) *
 			 (((double)temp_adc / 131072.0) - ((double)par_t[0] / 8192.0))) *
 			 ((double)par_t[2] * 16.0);
 	 temp_final = var1 + var2;
 	 temp_comp = temp_final / 5120.0; //Converts the temp to celcius
...
static double CalcHumidity(double temp_comp) {
 ...
 hum_adc = humidityData[0] << 8 | humidityData[1]; //Combine 2 bytes into 16-bit ADC value to use in calculations. 

 //Calculates the humidity compensation formula from the datasheet.
...
static void ProcessEnvData (void) {
 ...
 // Display temperature and relative humidity to 1 decimal place with units
 DisplayPrint(ENVIRO, 0, "Temp: %2.1f'C",temperature);
