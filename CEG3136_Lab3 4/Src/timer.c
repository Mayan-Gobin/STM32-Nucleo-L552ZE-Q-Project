...
void TimerEnable (TimerIO_t timer) {
 ...
 // Configure the GPIO pin using parameters from timer structure
 GPIO_Enable(timer.pin); //Turns on the power to the circuit
 GPIO_AltFunc(timer.pin, timer.af); //Connects  timer to the pin
 GPIO_Mode(timer.pin, ALTFUNC); //Switches the pin to the "timer output mode"
}
